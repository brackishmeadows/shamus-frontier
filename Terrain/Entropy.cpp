...
-------------------------------------------------------------------------------
  ...
  at non-powers-of-2 _sizes in order to avoid tiling as much as possible.
...
#define INDEX(x,y)        ((x % _size.x) + (y % _size.y) * _size.x)

static bool       loaded;
static GLcoord    _size;
static float*     emap;
...
static void entropy_erode ()
{
  ...
  buffer = new float[_size.x * _size.y];
  memcpy (buffer, emap, sizeof (float) * _size.x * _size.y);
  ...
  for (int pass = 0; pass < 3; pass++) {
    ...
    memcpy (emap, buffer, sizeof (float) * _size.x * _size.y);
  }
    ...
    for (x = 0; x < _size.x; x++) {
      ...
      for (n.x = -BLUR_RADIUS; n.x <= BLUR_RADIUS; n.x++) {
        for (n.y = -BLUR_RADIUS; n.y <= BLUR_RADIUS; n.y++) {
          current.x = ((x + n.x) + _size.x) % _size.x;
          current.y = ((y + n.y) + _size.y) % _size.y;
          index = entropy_index (current);
          val += buffer[index];
          count++;
        }
...
static void entropy_create (char* filename)
{
  ...
  if (!file)	
    return;
	fclose (file);
  buffer = FileImageLoad (filename, &_size);
  elements = _size.x * _size.y;
  emap = new float [elements];
  for (y = 0; y < _size.y; y++) {
    for (x = 0; x < _size.x; x++) {
      offset.x = (float)x / (float)_size.x;
      offset.y = (float)y / (float)_size.y;
      scan.x = (int)(offset.x * (float)_size.x);
      scan.y = (int)(offset.y * (float)_size.y);
      red = buffer[(scan.x + scan.y * _size.x) * 4];
      emap[x + y * _size.x] = (float)red / 255;
    }
  }
  entropy_erode ();
  file = fopen (ENTROPY_FILE, "wb");
  if (file) {
    fwrite (&_size.x, sizeof (_size.x), 1, file);
    fwrite (&_size.y, sizeof (_size.y), 1, file);
    fwrite (emap, sizeof (float), _size.x * _size.y, file);
    fclose (file);
  }
  delete buffer;
  loaded = true;
...
static void entropy_load ()
{
  ...
  if (file) {
    fread (&_size.x, sizeof (_size.x), 1, file);
    fread (&_size.y, sizeof (_size.y), 1, file);
    emap = new float [_size.x * _size.y];
    fread (emap, sizeof (float), _size.x * _size.y, file);
    fclose (file);
    loaded = true;
  } else
    entropy_create ("textures/noise256.bmp");
...
float Entropy (int x, int y)
{
  ...
  if (!emap || x < 0 || y < 0)
    return 0;
  return emap[(x % _size.x) + (y % _size.y) * _size.x];
